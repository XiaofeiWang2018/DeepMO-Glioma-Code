...
def train(opt):
    ...
    a=Mine_model.state_dict()
    for name, param in Mine_model.named_parameters():
        if not ("CDKN" in name or "1p19q" in name or "IDH" in name or "position" in name or "init" in name):
            param.requires_grad = False
    # paras=list(filter(lambda p: p.requires_grad, Mine_model.parameters()))
    opt_CDKN = torch.optim.Adam(filter(lambda p: p.requires_grad, Mine_model.parameters()), opt['Network']['lr'],weight_decay=0.00001)
    # Mine_model_sch_CDKN = get_scheduler(opt_CDKN, opt['n_ep'], opt['n_ep_decay'], opt['decayType'], -1) # the index but not the number of epoch, as last_ep-1
    ...
    if opt['resume_epoch']: # if resume the pre-trained Mine_model
        ...
        Mine_model.load_state_dict(checkpoint)
    ...
    for epoch in range(alleps):
        ...
        for packs in train_bar:
            ...
            pred=results_dict['logits'][2]


            loss_all = Mine_model.calculateLoss_CDKN(pred,label_CDKN)
            loss_all.backward()
            opt_CDKN.step()

            _, predicted = torch.max(pred.data, 1)
            total = label_CDKN.size(0)
            correct = predicted.eq(label_CDKN.data).cpu().sum()


            running_results['acc'] += 100. * correct / total
            running_results['allloss'] += loss_all.item()
            running_results['cls_loss'] += Mine_model.loss_CDKN.item()
            total_it = total_it + 1
            lossdict['train/cls'] += Mine_model.loss_CDKN.item()
        ...
        if (curep + 1) % opt['n_ep_save'] == 0:
            if (curep + 1) > (alleps / 2):
                ...
                state = {
                    ...
                    'optimizer': opt_CDKN.state_dict(),
                    'ep': curep + 1,
                    'total_it': total_it
                }
                torch.save(state, save_dir)
            print("----------Val-------------")
            list_WSI = validation_Binary_CDKN(opt, Mine_model, None, valLoader, saver, curep, opt['eva_cm'], gpuID)
            if opt['eva_cm']:
            ...
            print("----------Test-------------")
            list_WSI = validation_Binary_CDKN(opt, Mine_model, None, testLoader, saver, curep, opt['eva_cm'], gpuID)
