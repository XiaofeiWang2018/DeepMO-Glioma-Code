...
def train(opt):
    ...
    gpuID = opt['gpus']
    ############### Model #######################
    AMIL = amil(opt).cuda(gpuID[0])
    assert opt['name'].split('_')[0]=='AMIL'
    # print('# network parameters:', sum(param.numel() for param in AMIL.parameters()) / 1e6, 'M')
    ...
    model.init_weights(AMIL, init_type='xavier', init_gain=1)

    AMIL.to(device)
    AMIL_opt = torch.optim.Adam(filter(lambda p: p.requires_grad, AMIL.parameters()), opt['Network']['AMIL']['lr'], weight_decay=0.00001)
    ...
    if opt['resume_epoch']: # if resume the pre-trained model
        if opt['resume_epoch'] == -1:  # resume the last model
            ...
            AMIL.load_state_dict(checkpoint['network'])
            AMIL_opt.load_state_dict(checkpoint['optimizer'])
    ...
    if opt['decayType'] == 'exp' or opt['decayType'] == 'step':
        AMIL_sch = get_scheduler(AMIL_opt, opt['n_ep'], opt['n_ep_decay'], opt['decayType'], -1) # the index but not the number of epoch, as last_ep-1
    elif opt['decayType'] == 'cos':
        AMIL_sch  = WarmupCosineSchedule(AMIL_opt, warmup_steps=opt['decay_cos_warmup_steps'],t_total=opt['n_ep'])
    ...
    for epoch in range(alleps):
        ...
        for packs in train_bar:
            ...
            AMIL.train()
            AMIL.zero_grad()
            results_dict = AMIL(img)
            pred=results_dict['logits']

            loss_all = AMIL.calculateLoss(pred,label)
            loss_all.backward()
            AMIL_opt.step()
            ...
            total_it = total_it + 1
            lossdict['train/CE'] += AMIL.loss_ce.item()
        ...
        if (curep + 1) % opt['n_ep_save'] == 0:
            if (curep + 1) > (alleps / 2):
                ...
                state = {
                    'network': AMIL.state_dict(),
                    'optimizer': AMIL_opt.state_dict(),
                    'ep': curep + 1,
                    'total_it': total_it
                }
            ...
            if opt['name'].split('_')[1]=='Diag':
                list_WSI= validation_Diag(opt,AMIL,None, valLoader, saver, curep, opt['eva_cm'],gpuID)
            elif opt['name'].split('_')[1]=='IDH' or opt['name'].split('_')[1]=='1p19q' or opt['name'].split('_')[1]=='CDKN' or opt['name'].split('_')[1]=='His':
                list_WSI = validation_Binary(opt, AMIL, None, valLoader, saver, curep, opt['eva_cm'], gpuID)
            if opt['eva_cm']:
            ...
            print("----------Test-------------")
            if opt['name'].split('_')[1] == 'Diag':
                list_WSI = validation_Diag(opt, AMIL, None, testLoader, saver, curep, opt['eva_cm'], gpuID)
            elif opt['name'].split('_')[1] == 'IDH' or opt['name'].split('_')[1] == '1p19q' or opt['name'].split('_')[1] == 'CDKN' or opt['name'].split('_')[1]=='His':
                list_WSI = validation_Binary(opt, AMIL, None, testLoader, saver, curep, opt['eva_cm'], gpuID)
            if opt['eva_cm']:
                saver.write_cm_maps(curep, list_WSI[1], opt['dataLabels'], savename='cm_WSI.png')
