...
def train(opt):
    ...
    for name, param in Mine_model.named_parameters():
        if not ("Grade" in name or "His" in name or "init" in name or "position" in name):
            param.requires_grad = False
    for name, param in Mine_model.named_parameters():
        if ("encoder_norm_His" in name or "_fc2_His" in name):
            param.requires_grad = False
    opt_Grade = torch.optim.Adam(filter(lambda p: p.requires_grad, Mine_model.parameters()), opt['Network']['lr'],
                                 weight_decay=0.00001)

    Mine_model_sch_Grade = WarmupCosineSchedule(opt_Grade, warmup_steps=opt['decay_cos_warmup_steps'],
                                                t_total=opt['n_ep'])
    ...
    for epoch in range(alleps):
        ...
        for packs in train_bar:
            ...
            pred = results_dict['logits'][4]

            loss_all = Mine_model.calculateLoss_Grade(pred, label_Grade)
            loss_all.backward()
            opt_Grade.step()

            _, predicted = torch.max(pred.data, 1)
            total = label_Grade.size(0)
            correct = predicted.eq(label_Grade.data).cpu().sum()

            running_results['acc'] += 100. * correct / total
            running_results['allloss'] += loss_all.item()
            running_results['cls_loss'] += Mine_model.loss_Grade.item()
            total_it = total_it + 1
            lossdict['train/cls'] += Mine_model.loss_Grade.item()
        ...
        if (curep + 1) % opt['n_ep_save'] == 0:
            if (curep + 1) > (alleps / 2):
                ...
                state = {
                    ...
                    'optimizer': opt_Grade.state_dict(),
                    'ep': curep + 1,
                    'total_it': total_it
                }
                torch.save(state, save_dir)
            print("----------Val-------------")
            list_WSI = validation_Binary_Grade(opt, Mine_model, None, valLoader, saver, curep, opt['eva_cm'], gpuID)
            if opt['eva_cm']:
            ...
            print("----------Test-------------")
            list_WSI = validation_Binary_Grade(opt, Mine_model, None, testLoader, saver, curep, opt['eva_cm'], gpuID)
