...
def train(opt):
    ...
    # AMIL= model.init_weights(AMIL, init_type='xavier', init_gain=1)
    ...
    AMIL_sch = model.get_scheduler(AMIL_opt, opt['n_ep'], opt['n_ep_decay'], opt['decayType'], last_ep-1) # the index but not the number of epoch, as last_ep-1
    saver = Saver(opt)
    ...
    for epoch in range(alleps):
        ...
        for packs in train_bar:
            ...
            lossdict['train/IDH'] += AMIL.loss_ce_IDH.item()
            lossdict['train/1p19q'] += AMIL.loss_ce_1p19q.item()
            lossdict['train/CDKN'] += AMIL.loss_ce_CDKN.item()
            lossdict['train/Diag'] += AMIL.loss_ce_Diag.item()
        ...
        if (curep + 1) % opt['n_ep_save'] == 0:
            ...
            print("----------Val-------------")
            list_WSI_IDH,list_WSI_1p19q,list_WSI_CDKN,list_WSI_Diag= validation_MCL(opt,AMIL,None, valLoader, saver, curep, opt['eva_cm'],gpuID)
            if opt['eva_cm']:
            ...
            print("----------Test-------------")
            list_WSI_IDH,list_WSI_1p19q,list_WSI_CDKN,list_WSI_Diag = validation_MCL(opt, AMIL, None, testLoader, saver, curep, opt['eva_cm'], gpuID)
            if opt['eva_cm']:
                saver.write_cm_maps(curep, list_WSI_IDH[1], opt['dataLabels'], savename='cm_WSI_IDH.png')
                saver.write_cm_maps(curep, list_WSI_1p19q[1], opt['dataLabels'], savename='cm_WSI_1p19q.png')
                saver.write_cm_maps(curep, list_WSI_CDKN[1], opt['dataLabels'], savename='cm_WSI_CDKN.png')
