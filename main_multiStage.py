...
def train(opt):
    ...
    # ckptdir = os.path.join('./pretrain/', 'xx.pth')
    # checkpoint = torch.load(ckptdir)
    # related_params_His = {k: v for k, v in checkpoint['network'].items() if 'His' in k}
    # related_params.update(related_params_His)
    ...
    for epoch in range(alleps):
        ...
        for packs in train_bar:
            ...
            #
            # loss_IDH_subnet = loss_IDH + loss_1p19q + loss_CDKN+ 0.1 * loss_Graph
        ...
        if (curep + 1) % opt['n_ep_save'] == 0:
            ...
            print("----------Val-------------")
            list_WSI = validation_Stage1(opt, Mine_model_init, Mine_model_His, Mine_model_Cls, valLoader, opt['eva_cm'], gpuID)
            ...
            print("----------Test-------------")
            list_WSI = validation_Stage1(opt, Mine_model_init, Mine_model_His, Mine_model_Cls, testLoader, opt['eva_cm'], gpuID)
