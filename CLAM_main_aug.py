    ...
    for epoch in range(alleps):
        ...
        if (curep + 1) % opt['n_ep_save'] == 0:
            ...
            if opt['name'].split('_')[1]=='Diag':
                ...
                list_WSI= validation_His(opt,CLAM,None, valLoader, saver, curep, opt['eva_cm'],gpuID)
            elif opt['name'].split('_')[1]=='IDH' or opt['name'].split('_')[1]=='1p19q' or opt['name'].split('_')[1]=='CDKN':
